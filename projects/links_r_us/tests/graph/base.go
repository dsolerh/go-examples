...
func (s *Suite) TestConcurrentLinkIterators() {
	...
	for i := 0; i < numIterators; i++ {
		go func(id int) {
			...
			assert.Len(seen, numLinks, "iterator %d did not visit all the links", id)
			assert.Nil(it.Error(), "iterator %d had an error", id)
			assert.Nil(it.Close(), "iterator %d had an error closing", id)
		}(i)
	}
...
func (s *Suite) assertIteratedEdgeIDsMatch(assert *assert.Assertions, updatedBefore time.Time, exp []uuid.UUID) {
	...
	// should not produce an error
	assert.Nil(err)
	assertIteratedIDsMatch(assert, it, exp)
}
