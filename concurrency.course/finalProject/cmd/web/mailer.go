...
// a function to listn for messages
func (app *Config) listenForMails() {
	for {
		select {
...
func (m *MailServer) buildPlainMessage(msg Message) (string, error) {
	templateToRender := fmt.Sprintf("./cmd/web/templates/%s.html.gohtml", msg.Template)
	...
	plainMessage, err := m.inlineCSS(tpl.String())
	if err != nil {
		return "", err
	}
