...
// Setup initializes a config from flags that are pased in
func (c *Config) Setup() {
	// you can set the flag directly like so:
	...
	flag.BoolVar(&c.isAwesome, "isawesome", false, "it is awesome or what?")
	flag.IntVar(&c.howAwesome, "hawawesome", 10, "how awesome out of 10?")

	// custom variable type
	flag.Var(&c.countTheWays, "c", "coma separated list of integers")
}
...
func (c *Config) GetMessage() string {
	...
	return fmt.Sprintf(
		"%s with a certainty of %d out of 10. Let me count the ways %s",
		msg,
		c.howAwesome,
